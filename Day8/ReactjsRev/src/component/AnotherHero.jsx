...
const AnotherHero = (props) => {
    console.log(props)
  return (
    <div className="bg-[#EEF4FF] min-h-screen relative w-full text-black">
    <Nav propsitem = {props.channel} myName={props.myName.name}/>
    <Header />
  </div>
)
}

const Nav = ({propsitem,myName="test"}) => {
const navData = [
  {
    name: 'Home',
    href: '#home',
...
return (
  <div className="absolute top-0 z-50  w-full py-3 text-xl font-bold drop-shadow-xl flex-none transition-colors duration-500  ">
    <div className="px-4 flex justify-around">
      <div>
        <h1 className="font-jostBold text-4xl text-[#3E3FD8] ">{propsitem} , {myName}</h1>
      </div>
